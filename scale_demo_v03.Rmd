...
for (indicator in indicators_12) {
  for (subpop in subpops_12) {
    ...
    p1 <- indicator_plot(indicators_2012, subpop, indicator, measurements[indicator][[1]], scale_max[indicator][[1]]) + theme(plot.margin = unit(c(0,20,0,20), "pt"))
    p2 <- indicator_plot(indicators_2017, subpop, indicator, measurements[indicator][[1]], scale_max[indicator][[1]]) + theme(plot.margin = unit(c(0,20,0,20), "pt"))
    p <- p1 | p2
    print(p)
    cat("  \n")
  }
